...
ngx_http_upstream_cache_send(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    ngx_table_elt_t   *date_when_revalidated;
    ngx_int_t          rc;
    ngx_http_cache_t  *c;
    ...
    date_when_revalidated = u->headers_in.date;
    ...
    if (rc == NGX_OK) {
        if (u->cache_status == NGX_HTTP_CACHE_REVALIDATED) {
            ...
            u->headers_in.date = date_when_revalidated;
...
ngx_http_upstream_process_headers(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (r->cache) {
        ...
        if (u->cache_status == NGX_HTTP_CACHE_HIT
            || u->cache_status == NGX_HTTP_CACHE_STALE)
        {
            time_t  resident_time, age_value, current_age;
            ...
            r->headers_out.age_n = current_age ? current_age : -1;
                   ...
                   || u->cache_status == NGX_HTTP_CACHE_REVALIDATED)
        {
            time_t  response_time, age_value, date, response_delay,
             ...
             *
             * response_delay = min(max(0, response_time - date),
             *                      response_time - request_time);
             *
             * corrected_initial_age = age_value + response_delay;
             */
            ...
            response_delay = ngx_min(ngx_max(0, response_time - date),
                                     u->state->header_time / 1000);
            age_value = u->headers_in.age_n != -1 ? u->headers_in.age_n : 0;
            corrected_initial_age = age_value + response_delay;
